    ...
    sandesha2_msg_ctx_t *rm_msg_ctx)
{
    ...
    if(!csr_part)
    {
        ...
        AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_REQD_MSG_PART_MISSING, AXIS2_FAILURE);
        if(storage_mgr)
        {
    ...
    if(!relates_to)
    {
        ...
        AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_INVALID_RELATES_TO, AXIS2_FAILURE);
        if(storage_mgr)
        {
            sandesha2_storage_mgr_free(storage_mgr, env);
        }
