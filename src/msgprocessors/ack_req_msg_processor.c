    ...
    sandesha2_msg_ctx_t *rm_msg_ctx)
{
    ...
    if(!ack_requested)
    {
        ...
        AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_REQD_MSG_PART_MISSING,
            AXIS2_FAILURE);
    ...
    if(sandesha2_utils_is_anon_uri(env, acks_to_str))
    {
        ...
        if(AXIS2_FAILURE == axis2_engine_send(engine, env, ack_msg_ctx))
        {
            ...
            AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_SENDING_ACK, 
                AXIS2_FAILURE);
            if(engine)
            {
