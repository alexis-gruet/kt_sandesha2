    ...
    axis2_char_t *ns_val)
{
    ...
    {
        AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_UNSUPPORTED_NS, 
            AXIS2_FAILURE);
        return NULL;
    }    
    ...
    void *om_node)
{
    ...
    if(!seq_ack->identifier)
    {
        AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_TO_OM_NULL_ELEMENT, AXIS2_FAILURE);
        return NULL;
    }
    ...
    if(seq_ack->ack_none)
    {
        ...
        {
            AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_ACK_NONE_NOT_ALLOWED, AXIS2_FAILURE);
            return NULL;
        }
        else
        {
    ...
    if(seq_ack->ack_final)
    {
        if(!sandesha2_spec_specific_consts_is_ack_final_allowed(env, rm_spec_ver) ||  
                0 != axutil_array_list_size(seq_ack->nack_list, env))
        {
            AXIS2_ERROR_SET(env->error, (axutil_error_codes_t)SANDESHA2_ERROR_ACK_FINAL_NOT_ALLOWED, AXIS2_FAILURE);
            return NULL;
        }
        else
        {
